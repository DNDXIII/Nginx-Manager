...
RUN \
    apt-get update && \
    apt-get -y install ssh git libgd3 libxslt1.1 libgeoip1 && \
    mkdir -p /root/.ssh/ && \
    ...
    dpkg -i nginx-common_1.10.0-0ubuntu0.16.04.4-collab_all.deb nginx-full_1.10.0-0ubuntu0.16.04.4-collab_amd64.deb    && \
                rm /etc/nginx/nginx.conf && \
                cp nginx.conf /etc/nginx/nginx.conf && \
                rm /etc/nginx/sites-available/default && \
                cp default /etc/nginx/sites-available/default && \
                mkdir -p /etc/nginx/ssl/ && \
                cp -p  default.crt default.rsa /etc/nginx/ssl/ && \
